...
returns text as $$
  select role::text from "User" where id = auth.uid();
$$ language sql security definer stable;

create or replace function current_user_is_admin()
returns boolean as $$
  select exists(select 1 from "User" where id = auth.uid() and role = 'ADMIN');
$$ language sql security definer stable;
...
create policy "Customers: insert" on "Customer" for insert
  with check (owner_id = auth.uid() or current_user_is_manager_or_above());
...
create policy "FollowUps: select" on "FollowUp" for select
  using (assignee_id = auth.uid() or current_user_is_manager_or_above());
create policy "FollowUps: insert" on "FollowUp" for insert
  with check (assignee_id = auth.uid() or current_user_is_manager_or_above());
...
create policy "QuotationItems: select" on "QuotationItem" for select
  using (exists(select 1 from "Quotation" q where q.id = quotation_id and (q.owner_id = auth.uid() or current_user_is_manager_or_above())));
create policy "QuotationItems: insert" on "QuotationItem" for insert
  with check (exists(select 1 from "Quotation" q where q.id = quotation_id and (q.owner_id = auth.uid() or current_user_is_manager_or_above())));
...
create policy "OrderItems: select" on "OrderItem" for select
  using (exists(select 1 from "Order" o where o.id = order_id and (o.owner_id = auth.uid() or current_user_is_manager_or_above())));
create policy "OrderItems: insert" on "OrderItem" for insert
  with check (exists(select 1 from "Order" o where o.id = order_id and (o.owner_id = auth.uid() or current_user_is_manager_or_above())));
...
create policy "Calls: select" on "Call" for select
  using (user_id = auth.uid() or current_user_is_manager_or_above());
create policy "Calls: insert" on "Call" for insert
  with check (user_id = auth.uid() or current_user_is_manager_or_above());
...
create policy "Users: select" on "User" for select
  using (id = auth.uid() or current_user_is_manager_or_above());
